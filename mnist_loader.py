...
def loadImages(src, n_images=None):
    ...
    try:
        ...
        with gzip.open(gzfname) as gz:
            ...
            # least-significant-bytes first. So if the magic number (in hex format)
            # is 0x00000803 (2051 in decimal) then our Python code will actually
            # read in the last 4-bytes first, and then the first 4 bytes. That is, 
            # Python will load this value as 0x0803 followed by 0x0000. So this
            # is how we check for the correct magic number value.
            if n[0] != 0x03080000:
            ...
            if n_images == None:
                n_images = n    # Set the number of images to read to the number present
            elif n_images > n:
                raise Exception('Unable to read {0} entires from data file.'.format(n_images))
            # Now start loading images. The next 8 bytes (4 bytes at a time) 
            ...
            # The remainder of the data file is one continuous stream of binary data,
            # where every 28x28=784 bytes is one separate image. We will read
            # this data into one single array of bytes
...
def loadLabels(src, n_images=None):
    ...
    try:
        with gzip.open(gzfname) as gz:
            ...
            # Check that we haven't been asked to read in more than n images
            if n_images == None:
                n_images = n[0]    # Set the number of images to read to the number present
            elif n_images > n[0]:
                raise Exception('Unable to read {0} entires from label file.'.format(n_images))
            # Read labels. The remaining file is just a stream of integers that label the
            # corresponding images from the iamge file.
...
def mnist_load(imageSrc, labelsSrc, n_images):
    ...
    Inputs:
       ...
       labelSrc: The name of a file conttaining image labels
...
def vectorized_digit(j):
    '''
    Given a digt 'j', return a 10-element vector representing that digit.
